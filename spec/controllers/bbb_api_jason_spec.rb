...
RSpec.describe BigBlueButtonApiController, type: :request do
  ...
  # get_meeting_info
  describe 'GET #getMeetingInfo' do
    it("getMeetingInfo responds with the correct meeting info for a post request") do
      ...
      stub_request(:get,
                   url).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>test-meeting-1</meetingID></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
  ...
  describe 'POST #getMeetingInfo' do
    it("getMeetingInfo responds with the correct meeting info for a post request with checksum value computed using SHA1") do
      ...
      stub_request(:get,
                   url).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>SHA1_meeting</meetingID></response>")
      Rails.configuration.x.stub(:loadbalancer_secrets, ["test-2"]) do
        post(bigbluebutton_api_get_meeting_info_url,
             params: { meetingID: "SHA1_meeting", checksum: "cbf00ea96fae6ff06c2cb311bbde8b26ad66d765" })
      end
      response_xml = Nokogiri.XML(@response.body)
      expect(response_xml.at_xpath("/response/returncode").content).to(eq("SUCCESS"))
    ...
    it("getMeetingInfo responds with the correct meeting info for a post request with checksum value computed using SHA256") do
      ...
      stub_request(:get,
                   url).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>SHA256_meeting</meetingID></response>")
  ...
  # is_meeting_running
  describe 'GET #isMeetingRunning' do
    it("isMeetingRunning responds with the correct meeting status for a get request") do
      ...
      encoded_uri = encode_bbb_uri("isMeetingRunning", server1.url, server1.secret, "meetingID" => meeting1.id)
      stub_request(:get, encoded_uri).to_return(body: "<response><returncode>SUCCESS</returncode><running>true</running></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
    ...
    it("isMeetingRunning responds with appropriate error on timeout") do
      ...
      meeting1 = Meeting.find_or_create_with_server("Demo Meeting", server1, "mp")
      encoded_uri = encode_bbb_uri("isMeetingRunning", server1.url, server1.secret, "meetingID" => meeting1.id)
      stub_request(:get, encoded_uri).to_timeout
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
      get(bigbluebutton_api_is_meeting_running_url, params: { meetingID: meeting1.id })
      response_xml = Nokogiri.XML(@response.body)
      expect(response_xml.at_xpath("/response/returncode").content).to(eq("FAILED"))
  ...
  describe 'POST #isMeetingRunning' do
  ...
  describe 'GET #get_meetings' do
    it("getMeetings responds with the correct meetings  for a get request") do
      ...
      server2 = Server.create(url: "https://test-2.example.com/bigbluebutton/api", secret: "test-2-secret", load: 1,
                              online: true, enabled: true)
      encoded_uri_server1 = encode_bbb_uri("getMeetings", server1.url, server1.secret)
      encoded_uri_server2 = encode_bbb_uri("getMeetings", server2.url, server2.secret)
      stub_request(:get, encoded_uri_server1).to_return(body: "<response><returncode>SUCCESS</returncode><meetings><meeting>test-meeting-1<meeting></meetings></response>")
      stub_request(:get, encoded_uri_server2).to_return(body: "<response><returncode>SUCCESS</returncode><meetings><meeting>test-meeting-2<meeting></meetings></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
    ...
    it("getMeetings responds with appropriate error on timeout") do
      ...
      server2 = Server.create(url: "https://test-2.example.com/bigbluebutton/api", secret: "test-2-secret", load: 1,
                              online: true, enabled: true)
      server3 = Server.create(url: "https://test-3.example.com/bigbluebutton/api", secret: "test-3-secret", load: 1,
                              online: true, enabled: true)
      encoded_uri_server1 = encode_bbb_uri("getMeetings", server1.url, server1.secret)
      encoded_uri_server2 = encode_bbb_uri("getMeetings", server2.url, server2.secret)
      encoded_uri_server3 = encode_bbb_uri("getMeetings", server3.url, server3.secret)
      stub_request(:get, encoded_uri_server1).to_return(body: "<response><returncode>SUCCESS</returncode><meetings><meeting>test-meeting-1<meeting></meetings></response>")
      stub_request(:get, encoded_uri_server2).to_timeout
      stub_request(:get, encoded_uri_server3).to_return(body: "<response><returncode>SUCCESS</returncode><meetings><meeting>test-meeting-3<meeting></meetings></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
      get(bigbluebutton_api_get_meetings_url)
      response_xml = Nokogiri.XML(@response.body)
      expect(response_xml.at_xpath("/response/returncode").content).to(eq("FAILED"))
    ...
    it("getMeetings only makes a request to online and enabled servers") do
      ...
      expect(response_xml.xpath("//meeting[text()=\"test-meeting-1\"]").present?).to(eq(true))
      expect(response_xml.xpath("//meeting[text()=\"test-meeting-2\"]").present?).to(eq(true))
      expect(response_xml.xpath("//meeting[text()=\"test-meeting-3\"]")).to be_nil
    end

    it("getMeetings only makes a request to online and servers in state cordoned/enabled") do
      server1 = Server.create(url: "https://test-1.example.com/bigbluebutton/api", secret: "test-1-secret", load: 1,
                              online: true, state: "cordoned")
      server2 = Server.create(url: "https://test-2.example.com/bigbluebutton/api", secret: "test-2-secret", load: 1,
      ...
      stub_request(:get,
                   encode_bbb_uri("getMeetings", server1.url,
                                  server1.secret)).to_return(body: "<response><returncode>SUCCESS</returncode><meetings><meeting>test-meeting-1<meeting></meetings></response>")
      stub_request(:get,
                   encode_bbb_uri("getMeetings", server2.url,
                                  server2.secret)).to_return(body: "<response><returncode>SUCCESS</returncode><meetings><meeting>test-meeting-2<meeting></meetings></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
      get(bigbluebutton_api_get_meetings_url)
      response_xml = Nokogiri.XML(@response.body)
      expect(response_xml.at_xpath("/response/returncode").text).to(eq("SUCCESS"))
  ...
  describe 'GET #create' do
    ...
    it("create creates the room successfully for a get request") do
      server1 = Server.create(url: "https://test-1.example.com/bigbluebutton/api/", secret: "test-1-secret",
                              enabled: true, load: 0)
      params = { meetingID: "test-meeting-1", moderatorPW: "mp" }
      stub_request(:get,
                   encode_bbb_uri("create", server1.url, server1.secret,
                                  params)).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>test-meeting-1</meetingID><attendeePW>ap</attendeePW><moderatorPW>mp</moderatorPW><messageKey/><message/></response>")
      BigBlueButtonApiController.stub_any_instance(:verify_checksum, nil) do
    ...
    it("create increments the server load by the value of load_multiplier") do
      server1 = Server.create(url: "https://test-1.example.com/bigbluebutton/api/", secret: "test-1-secret",
                              enabled: true, load: 0, load_multiplier: 7.0)
      params = { meetingID: "test-meeting-1", moderatorPW: "mp" }
      stub_request(:get,
                   encode_bbb_uri("create", server1.url, server1.secret,
                                  params)).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>test-meeting-1</meetingID><attendeePW>ap</attendeePW><moderatorPW>mp</moderatorPW><messageKey/><message/></response>")
    ...
    it("create creates the room successfully  with only permitted params for create") do
      ...
      stub_request(:get,
                   encode_bbb_uri("create", server1.url, server1.secret,
                                  filtered_params)).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>test-meeting-1</meetingID><attendeePW>ap</attendeePW><moderatorPW>mp</moderatorPW><messageKey/><message/></response>")
    ...
    it("create creates a record in callback_data if  params[\"meta_bn-recording-ready-url\"] is present in request") do
      ...
      stub_request(:get,
                   encode_bbb_uri("create", server1.url, server1.secret,
                                  bbb_params)).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>test-meeting-1</meetingID><attendeePW>ap</attendeePW><moderatorPW>mp</moderatorPW><messageKey/><message/></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
      get(bigbluebutton_api_create_url, params: params)
      response_xml = Nokogiri.XML(@response.body)
    ...
    it("create creates a record in callback_data if  params[\"meta_analytics-callback-url\"] is present in request") do
      server1 = Server.create(url: "https://test-1.example.com/bigbluebutton/api/", secret: "test-1-secret",
                              enabled: true, load: 0)
      params = { :meetingID => "test-meeting-66", :test4 => "", :test2 => "", :moderatorPW => "test-password",
  ...
  # analytics_callback
  describe 'POST #analytics_callback' do
    it("analytics_callback makes a callback to the specific meetings analytics_callback_url") do
      server1 = Server.create(url: "https://test-1.example.com/bigbluebutton/api/", secret: "test-1-secret",
                              enabled: true, load: 0)
      params = { :meetingID => "test-meeting-1111", :test4 => "", :test2 => "", :moderatorPW => "test-password",
                 "meta_analytics-callback-url" => "https://test.scalelite.com/bigbluebutton/api/analytics_callback" }
      stub_request(:get,
                   encode_bbb_uri("create", server1.url, server1.secret,
                                  params)).to_return(body: "<response><returncode>SUCCESS</returncode><meetingID>test-meeting-1</meetingID><attendeePW>ap</attendeePW><moderatorPW>mp</moderatorPW><messageKey/><message/></response>")
      stub_request(:post, "https://test.scalelite.com/bigbluebutton/api/analytics_callback").to_return(status: :ok,
                                                                                                       body: "", headers: {})
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
      Rails.configuration.x.stub(:url_host, "test.scalelite.com") do
        get(bigbluebutton_api_create_url, params: params)
  ...
  describe 'GET #end' do
    ...
    it("end responds with MeetingNotFoundError if meetingID && password are passed but meeting doesnt exist") do
      ...
      stub_request(:get,
                   encode_bbb_uri("end", server1.url, server1.secret,
                                  params)).to_return(body: "<response><returncode>FAILED</returncode><messageKey>notFound</messageKey><message>We could not find a meeting with that meeting ID - perhaps the meeting is not yet running?</message></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
    ...
    it("end responds with sentEndMeetingRequest if meeting exists and password is correct for a get request") do
      ...
      stub_request(:get,
                   encode_bbb_uri("end", server1.url, server1.secret,
                                  params)).to_return(body: "<response><returncode>SUCCESS</returncode><messageKey>sentEndMeetingRequest</messageKey><message>A request to end the meeting was sent. Please wait a few seconds, and then use the getMeetingInfo or isMeetingRunning API calls to verify that it was ended.</message></response>")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
      get(bigbluebutton_api_end_url, params: params)
      response_xml = Nokogiri.XML(@response.body)
      expect(response_xml.at_xpath("/response/returncode").text).to(eq("SUCCESS"))
  ...
  describe 'GET #join' do
    ...
    it("join redirects user to the corrent join url for a get request") do
      server1 = Server.create(url: "https://test-1.example.com/bigbluebutton/api/", secret: "test-1-secret",
                              enabled: true, load: 0, online: true)
    ...
    it("join redirects user to the current join url with given params if excluded params list is empty") do
      ...
      get(bigbluebutton_api_join_url, params: params)
      filtered_params = { meetingID: meeting.id, password: "test-password", fullName: "test-name" }
      expect(%w[test1 test2]).to(eq(Rails.configuration.x.join_exclude_params))
      assert_redirected_to(encode_bbb_uri("join", server1.url, server1.secret, filtered_params).to_s)
    end

    it("join responds with ServerUnavailableError if server is disabled") do
      server1 = Server.create(url: "https://test-1.example.com/bigbluebutton/api/", secret: "test-1-secret",
                              enabled: false, load: 0, online: true)
      Meeting.find_or_create_with_server("test-meeting-1", server1, "mp")
      allow_any_instance_of(BigBlueButtonApiController).to receive(:verify_checksum).and_return(nil)
      get(bigbluebutton_api_join_url, params: { meetingID: "test-meeting-1" })
      response_xml = Nokogiri.XML(@response.body)
  ...
  # get_recordings
  describe 'GET #get_recordings' do
  ...
  describe 'POST #get_recordings' do
  ...
  # publish_recordings
  describe 'GET #publish_recordings' do
  ...
  describe 'POST #publish_recordings' do
  ...
  # update_recordings
  describe 'GET #update_recordings' do
    ...
    it("updateRecordings adds a new meta parameter") do
      ...
      params = encode_bbb_params("updateRecordings", { recordID: r.record_id }.merge(meta_params.transform_keys { |k|
        "meta_#{k}"
      }).to_query)
    ...
    it("updateRecordings updates metadata on multiple recordings") do
      r1 = create(:recording_with_metadata,
                  meta_params: { "isBreakout" => "false", "meetingName" => "Fred's Room", "gl-listed" => "false" })
      r2 = create(:recording)
      meta_params = { "newkey" => "newvalue", "gl-listed" => "" }
      params = encode_bbb_params("updateRecordings", { recordID: "#{r1.record_id},#{r2.record_id}" }.merge(meta_params.transform_keys { |k|
        "meta_#{k}"
      }).to_query)
  ...
  describe 'POST #update_recordings' do
  ...
  # delete_recordings
  describe 'GET #delete_recordings' do
  ...
  describe 'POST #delete_recordings' do
